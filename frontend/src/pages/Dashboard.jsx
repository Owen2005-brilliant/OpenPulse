...
function Panel({ title, children, className = "", compact = false }) {
  return (
    <div
      className={`relative overflow-hidden rounded-xl border border-white/10 bg-gradient-to-b from-slate-900/45 to-slate-950/20 backdrop-blur-md shadow-[0_0_60px_rgba(59,130,246,0.10)] hover:shadow-[0_0_90px_rgba(56,189,248,0.16)] transition-shadow
      after:content-[''] after:pointer-events-none after:absolute after:inset-x-0 after:top-0 after:h-px after:bg-gradient-to-r after:from-transparent after:via-sky-400/30 after:to-transparent
      ${className}`}
    >
      {title ? (
...
function StatCard({ label, value, suffix = "", trend = "up", compact = false }) {
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl border border-white/10 bg-gradient-to-b from-slate-900/45 to-slate-950/20 backdrop-blur-md shadow-[0_0_50px_rgba(59,130,246,0.10)] hover:shadow-[0_0_80px_rgba(56,189,248,0.16)] transition-shadow ${
    ...
    >
      ...
      <div className={`${compact ? "mt-2" : ""} flex items-end justify-center gap-2`}>
        <div
          className={`font-extrabold leading-none bg-gradient-to-r from-slate-100 via-sky-100 to-slate-100 bg-clip-text text-transparent ${
...
function TrendChart({ series }) {
  ...
  const option = {
    ...
    grid: { left: 44, right: 20, top: 58, bottom: 42, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: { color: "#94a3b8", hideOverlap: true, margin: 10, fontSize: 11 },
...
function ActivityOpenRankChart({ series }) {
  ...
  const option = {
    ...
    grid: { left: 40, right: 20, top: 58, bottom: 40, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: { color: "#94a3b8", hideOverlap: true, margin: 10, fontSize: 11 },
...
function ResponsivenessChart({ series }) {
  ...
  const option = {
    tooltip: { trigger: "axis" },
    legend: {
      data: ["Issue Response Median (h)", "PR Resolution Median (h)"],
      top: 6,
      left: "center",
      type: "scroll",
      itemWidth: 14,
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 py-4">
        {/* Header */}
        <div className="op-glass-card op-glow-border relative overflow-hidden rounded-xl border border-white/10 bg-gradient-to-b from-slate-900/45 to-slate-950/20 backdrop-blur-md px-6 py-4 text-center shadow-[0_0_100px_rgba(56,189,248,0.16)]">
          <div className="text-xs text-slate-300 tracking-[0.22em] uppercase">
            Open Source Intelligence · Metrics · AI Governance
          </div>
          <div className="mt-2 text-2xl md:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-sky-300 via-blue-200 to-violet-300 bg-clip-text text-transparent">
            OpenPulse — Project Health Monitoring and Governance Decision Support Platform
          </div>
          <div className="mt-3 h-px w-[520px] max-w-full mx-auto bg-gradient-to-r from-transparent via-sky-400/35 to-transparent" />
        </div>
